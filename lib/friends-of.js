...
function friendsOfRecursion(name, adjacencyList, visited, maxDistance, currentDistance) {
  if (currentDistance <= maxDistance) {
    for (let neighbor of adjacencyList[name]){
      visited.add(neighbor);
      friendsOfRecursion(neighbor, adjacencyList, visited, maxDistance, currentDistance + 1);
    }
  }
}
...
function friendsOf(adjacencyList, name, distance) {
  if (name in adjacencyList){
    let visited = new Set();
    friendsOfRecursion(name, adjacencyList, visited, distance, 1);
    visited.delete(name);
